...
class WeatherWorker(QObject):
    ...
    def __init__(self, city):
        super().__init__(); self.city = city
    def run(self):
        try:
            city = urllib.parse.quote(self.city.strip())
            req = urllib.request.Request(f"https://wttr.in/{city}?format=j1", headers={"User-Agent": "PIXEL/9.0"})
...
class Pixel(QWidget):
    ...
    def build_ui(self):
        ...
        wh = QHBoxLayout(); wt = QLabel("☁ HAVA DURUMU"); wt.setObjectName("section"); wh.addWidget(wt); wh.addStretch(); self.weather_city=QLabel(self.config["weather_city"].upper()); self.weather_city.setObjectName("tiny"); wh.addWidget(self.weather_city); wl.addLayout(wh)
    ...
    def refresh_weather(self):
        ...
        self.weather_thread=QThread(self); self.weather_worker=WeatherWorker(self.config["weather_city"]); self.weather_worker.moveToThread(self.weather_thread)
    ...
    def change_city(self):
        ...
        if ok and city.strip(): self.config["weather_city"]=city.strip(); save_config(self.config); self.weather_city.setText(city.strip().upper()); self.refresh_weather()
    def edit_note(self):
        text,ok=QInputDialog.getText(self,"MİNİ GÖREV","GÖREVİN NE?",text=self.note.text())
        if ok and text.strip(): text=text.strip().upper(); self.note.setText(text); self.config["note"]=text; save_config(self.config)
    ...
    def open_settings(self):
        ...
        v=dlg.values(); old_top=self.config.get("always_on_top",False); old_city=self.config.get("weather_city")
        self.config.update({k:v[k] for k in ("weather_city","sleep_after","notifications","random_reactions","always_on_top")})
        self.set_autostart(v["autostart"]); self.apply_theme_preset(v["theme_name"]); save_config(self.config)
        self.weather_city.setText(self.config["weather_city"].upper()); self.configure_notification_watcher()
        if self.config["weather_city"] != old_city: self.refresh_weather()
